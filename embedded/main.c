...
#define CMD_LEN     3   // Command name is the first three characters
#define PNG         1
#define ABT         2
#define RES         3
#define MOV         9
...
/* Seek through buffer until a zero is found */
void seekcmd(char *buffer, int *numchars) {
    // Seek until first zero
    while ((*buffer++ != 0) && (*numchars + 2 <= SCI_BUFSIZ)) (*numchars)++;
    
    // Seek until non-zero is found. (beginning of new cmd)
    while ((*buffer++ == 0) && (*numchars + 2 <= SCI_BUFSIZ)) (*numchars)++;
}
