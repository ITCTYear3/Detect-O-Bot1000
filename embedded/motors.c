...
long read_period1 = 0, read_period2 = 0;                    // Calculated period using linear conversion equation
long speed_error1 = 0, speed_error2 = 0;                    // Difference between the desired set point and the read period
long intermediate_drive_value1, intermediate_drive_value2;  // Drive value before limit checking
byte drive_value1 = 0, drive_value2 = 0;                    // Value written to PWM duty register
...
void motor_set_speed(byte motor, char speed) {
    ...
    case MOTOR1C:
        if (speed == 0) {
            ...
            set_point1 = ((abs(speed) * (MAXDRIVE - MINDRIVE1)) / SPEED_SET_SCALE) + MINDRIVE1;     // Set the setpoint of the left motor.
    ...
    case MOTOR2C:
        if (speed == 0) {
            ...
            set_point2 = ((abs(speed) * (MAXDRIVE - MINDRIVE2)) / SPEED_SET_SCALE) + MINDRIVE2;     // Set the setpoint of the left motor.
...
interrupt VECTOR_NUM(TC_VECTOR(TC_MOTOR)) void Control_Law_ISR(void) {
    dword encoder_period1 = 0, encoder_period2 = 0;         // Local var for encoder period
    // Speed and integral error must be a signed value, integral error must be static
    static long integral_error1 = 0, integral_error2 = 0;
    
    // *** speed error and drive value moved to be global vars
    //long read_period1 = 0, read_period2 = 0;      // Calculated period using linear conversion equation
    //long speed_error1 = 0, speed_error2 = 0;      // Difference between the desired set point and the read period
    //long intermediate_drive_value1, intermediate_drive_value2;    // Drive value before limit checking
    //byte drive_value1 = 0, drive_value2 = 0;      // Value written to PWM duty register
    ...
    encoder_period1 = encoder_period(ENC1);
    encoder_period2 = encoder_period(ENC2);
    
    // Calculate error
    read_period1 = (DRIVE_SCALE_VAL / encoder_period1);
    read_period1 *= (MINDRIVE1 - MAXDRIVE);
    read_period1 /= (MINFREQ1 - MAXFREQ1);
    read_period1 += BVALUE1;
    
    read_period2 = (DRIVE_SCALE_VAL / encoder_period2);
    read_period2 *= (MINDRIVE2 - MAXDRIVE);
    read_period2 /= (MINFREQ2 - MAXFREQ2);
    read_period2 += BVALUE2;
    ...
    if((speed_error1 >= -MAX_SPEED_ERROR) && (speed_error1 <= MAX_SPEED_ERROR)) {
        ...
        if (!((PWM_DTY(MOTOR1_PWM) <= MINDRIVE1) && (speed_error1 < 0)) && !((PWM_DTY(MOTOR1_PWM) >= MAXDRIVE) && (speed_error1 > 0)))
            integral_error1 += speed_error1;
        
        // Perform control law calculation
        intermediate_drive_value1 = (((speed_error1 * P_GAIN) + (integral_error1 * I_GAIN)) / GAIN_DIV);
        
        // Bind to limits
        if(intermediate_drive_value1 < 0) {
            drive_value1 = 0;
        } else if(intermediate_drive_value1 > MOTOR_PER_DEF) {
            drive_value1 = MOTOR_PER_DEF;
        } else {            
    ...
    if((speed_error2 >= -MAX_SPEED_ERROR) && (speed_error2 <= MAX_SPEED_ERROR)) {
        // Check if integration needs to be performed (not at rails)
        if (!((PWM_DTY(MOTOR2_PWM) <= MINDRIVE1) && (speed_error2 < 0)) && !((PWM_DTY(MOTOR2_PWM) >= MAXDRIVE) && (speed_error2 > 0)))
            integral_error2 += speed_error2;
        
        // Perform control law calculation
        intermediate_drive_value2 = (((speed_error2 * P_GAIN) + (integral_error2 * I_GAIN)) / GAIN_DIV);
        
        // Bind to limits
        if(intermediate_drive_value2 < 0) {
            drive_value2 = 0;
        } else if(intermediate_drive_value2 > MOTOR_PER_DEF) {
            drive_value2 = MOTOR_PER_DEF;
        } else {            
